...
class Cube():
    ...
    def displayAll(self):
        def disp(face):
            ...
            for i in range(9):
                ...
                img = show(self.state[face][i//3][i%3])
                ax1.imshow(img, interpolation='None')
                plt.suptitle(face.capitalize())
            
            plt.gcf().canvas.set_window_title(face.capitalize())
    ...
    #find location of edge cubie on cube
    ...
    def solve(self):
        print("\n---Cross---\n")
        self.cross()
        assert(self.verify_cross()), "Unsolvable"
        print("\n---First layer corners---\n")
        self.first_layer_corners()
        assert(self.verify_corners()), "Unsolvable"
        print("\n---Second layer---")
        print("Rotate cube\n")
        self.up_down()
        self.second_layer()
        assert(self.verify_f2l()), "Unsolvable"
        print("\n---OLL---\n")
        self.oll()
        assert(self.verify_oll()), "Unsolvable"
        print("\n---PLL---\n")
        self.pll()
        assert(self.verify_pll()), "Unsolvable"
